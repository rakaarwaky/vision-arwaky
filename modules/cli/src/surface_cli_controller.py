...
def create_parser() -> argparse.ArgumentParser:
    ...
    analyze.add_argument("--image", required=True, help=_HELP_IMAGE_PATH)
    ...
    ocr.add_argument("--image", required=True, help=_HELP_IMAGE_PATH)
    ...
    video_info.add_argument("--video", required=True, help=_HELP_VIDEO_PATH)

    extract = subparsers.add_parser("extract-frames", help="Extract frames from video")
    extract.add_argument("--video", required=True, help=_HELP_VIDEO_PATH)

    corruption = subparsers.add_parser(
        "check-corruption", help="Check if video is corrupted"
    )
    corruption.add_argument("--video", required=True, help=_HELP_VIDEO_PATH)

    # --- Analysis commands ---
    scenes = subparsers.add_parser(
        "detect-scenes", help="Detect scene changes in video"
    )
    scenes.add_argument("--video", required=True, help=_HELP_VIDEO_PATH)

    motion = subparsers.add_parser(
        "detect-motion", help="Detect motion events in video"
    )
    motion.add_argument("--video", required=True, help=_HELP_VIDEO_PATH)

    track = subparsers.add_parser("track", help="Track object through video")
    track.add_argument("--video", required=True, help=_HELP_VIDEO_PATH)
    ...
    analyze_video.add_argument("--video", required=True, help=_HELP_VIDEO_PATH)
