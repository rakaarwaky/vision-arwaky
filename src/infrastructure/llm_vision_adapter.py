...
2. External: OpenAI-compatible APIs (LM Studio, Ollama, vLLM).
...
from src.taxonomy import VisionAnalysis
...
class LLMVisionAdapter(LLMVisionPort):
    ...
    def __init__(
        ...
        # 1. Load config.yaml from project root if it exists
        ...
        if config_path.exists():
            try:
                ...
                logger.warning(f"Failed to read config.yaml: {e}. Falling back to default environment variables.")
        ...
        url = base_url or os.getenv("LM_STUDIO_URL") or self._get_nested_config("external", "url") or DEFAULT_URL
        self.base_url = url.rstrip("/")
        self.api_key = api_key or os.getenv("LM_STUDIO_API_KEY") or self._get_nested_config("external", "api_key") or DEFAULT_API_KEY
        self._model = model or os.getenv("LM_STUDIO_MODEL") or self._get_nested_config("external", "model") or ""
        ...
        if self.backend == "native":
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    "Ensure LM Studio / Ollama is running and a vision model is loaded."
                ) from e
