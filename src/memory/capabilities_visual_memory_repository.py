...
from src.shared.system_utils_port import SystemUtilsPort
from src.shared.vision_models_vo import MemoryEntry, FilePath, MemoryLabel, DistanceThreshold


# Allowed directories for memory operations
ALLOWED_DIRS = [
    os.path.expanduser("~"),
    "/tmp",
    "/var/tmp",
...
class VisualMemoryStore(VisualMemoryProtocol):
    ...
    def _save_index(self):
        import tempfile
        fd, tmp = tempfile.mkstemp(dir=self.MEMORY_DIR, suffix=".tmp")
        try:
    ...
    def remember_image(self, image_path: FilePath, label: MemoryLabel) -> MemoryEntry:
        ...
        abs_path = _validate_path(image_path.value)
    ...
    def find_similar_images(self, query_image_path: FilePath, max_distance: DistanceThreshold) -> List[MemoryEntry]:
        ...
        abs_path = _validate_path(query_image_path.value)
        if not os.path.exists(abs_path):
            raise FileNotFoundError(f"Image not found: {abs_path}")
